...
@bp.route('/upload', methods=['POST']) #push file to bucket, and return file id
def upload_bucket():
...
@bp.route('/get/<filename>', methods=['GET']) #Get file from bucket and return as response, this is for testing purpose, not recommended for large files
...
@bp.route('/download/<filename>', methods=['GET']) #download file from bucket, this is for testing purpose, not recommended for large files
def get_fs(filename):
    if session.get('authenticated'):
...
@bp.route('/put', methods=['POST']) #push file to bucket, and return file id, this is for testing purpose, not recommended for large files
def put_fs():
    if 'file' in request.files and session.get('authenticated'):
        fs = GridFS(Database.get_connection())
        file = request.files['file']
...
@bp.route('/stream/<filename>', methods=['GET']) #stream file from bucket, this is for testing purpose, not recommended for large files, this is for streaming large files without loading them into memory
